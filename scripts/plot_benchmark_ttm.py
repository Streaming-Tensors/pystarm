...
Each subplot: two lines (batched, loop), x = thread count (log2), y = mean time (s).
...
COLORS = {"batched": "#4C72B0", "loop": "#DD8452"}
LSTYLE = {"batched": "-",       "loop": "--"}
MARKER = {"batched": "o",       "loop": "s"}
...
for dname, cfg in DATASETS.items():
    ...
    fig.suptitle(f"TTM batched vs loop — {dname}", fontsize=8, y=1.01)
    plt.tight_layout()
