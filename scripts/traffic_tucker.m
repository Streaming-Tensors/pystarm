% traffic-tucker.m
%
% Runs a single HOSVD experiment on the traffic dataset (color or grayscale)
% for one (tol, perm_str, dname_str) combination. Prints results to stdout
% in the same format as experiments.py so that parse_logs.py can parse the
% log file without any changes.
%
% Called by experiments.sh once per parameter combination. The caller sets
% tol, perm_str, and dname_str in the workspace via the -r flag:
%
%   matlab -nodisplay -nosplash -r \
%     "addpath('...'); tol=0.0001; perm_str='0123'; dname_str='traffic-color'; \
%      run('/path/to/scripts/traffic-tucker.m'); exit"
%
% Inputs (set in workspace by caller):
%   tol       - relative energy tolerance (e.g. 0.0001)
%   perm_str  - permutation mode string ('0123' or '0321' for color;
%               '012' or '021' for grayscale)
%   dname_str - dataset name: 'traffic-color' (4-way) or 'traffic-gray' (3-way)
%
% Requirements:
%   - MATLAB Tensor Toolbox on the MATLAB path (addpath before calling)
%   - data/traffic.bin present relative to the code root directory

% --- Locate data file relative to this script ---
script_dir = fileparts(mfilename('fullpath'));
data_file  = fullfile(script_dir, '..', 'data', 'traffic.bin');
...
% Reshape into 4D color tensor (H x W x 3 x T)
color = reshape(data, [height, width, 3, numFrames]);

% --- Build tensor and permutation based on dname_str ---
...
elseif strcmp(dname_str, 'traffic-gray')
...
fprintf('tol          : %g\n', tol);
fprintf('dname        : %s\n', dname_str);
fprintf('dfile        : data/traffic.bin\n');
fprintf('perm_mode    : %s\n', perm_label);
